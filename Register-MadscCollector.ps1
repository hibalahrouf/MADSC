...
if (-not $isAdmin) { Write-Host "[X] Exécuter en PowerShell ELEVÉ (administrateur)." -ForegroundColor Red; exit 1 }

if ($Remove) {
    if (Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue) {
        Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false
        Write-Host "[OK] Tâche '$TaskName' supprimée (rollback)." -ForegroundColor Green
    } else {
        Write-Host "[i] Aucune tâche '$TaskName' à supprimer." -ForegroundColor Yellow
    }
    return
}
...
Write-Host "[OK] Tâche planifiée '$TaskName' enregistrée." -ForegroundColor Green
Write-Host "     Collecteur : $CollectorPath"
Write-Host "     Sortie     : $OutDir"
Write-Host "     Fréquence  : toutes les $IntervalHours h (compte SYSTEM, lecture seule)"
Write-Host "     Rollback   : .\Register-MadscCollector.ps1 -Remove" -ForegroundColor Cyan

# Première collecte immédiate (produit un premier snapshot tout de suite)
Start-ScheduledTask -TaskName $TaskName
Write-Host "[i] Première collecte lancée. Vérifie $OutDir dans quelques secondes." -ForegroundColor Cyan
