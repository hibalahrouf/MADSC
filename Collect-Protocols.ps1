...
.DESCRIPTION
    Lit l'état de durcissement des protocoles (LDAP/SMB/NTLM/LLMNR) et produit un
    snapshot JSON consommable par le backend MADSC.

    GARANTIE : ce script est 100 % LECTURE. Il n'utilise que Get-* et des lectures
    de registre. Aucune écriture AD / registre / GPO. Exécutable sans risque sur DC-01,
    PMS-01 ou WS-01. (Ne dépend PAS du service EventLog : fonctionne même sur PMS-01.)

.PARAMETER OutDir
    Dossier de sortie du snapshot JSON (défaut : dossier courant).
